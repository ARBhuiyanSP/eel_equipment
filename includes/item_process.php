...
/*-----------level 4-----------*/
/*-----------level 5-----------*/
if(isset($_GET['process_type']) && $_GET['process_type'] == 'getMatbyParent'){
    include '../connection/connect.php';
    include '../helper/utilities.php';
    $level_1_id_l5      =    $_POST['level_1_id_l5'];
    $tableName      =    'inv_material where material_id='.$level_1_id_l5;
    $tableData      = getTableDataByTableName($tableName, '', 'material_description');
    if (isset($tableData) && !empty($tableData)) {
        echo "<option value=''>Please Select</option>";
        foreach ($tableData as $data) { ?>
            <option value="<?php echo $data['id']; ?>"><?php echo $data['material_description'].'('.$data['material_id_code'].')'; ?></option>
            <?php
        }
    }
}
