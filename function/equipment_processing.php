...
if (isset($_POST['equipment_entry']) && !empty($_POST['equipment_entry'])){
    ...
    $rrr_info_response  =   execute_equipment_assign_table();
    ...
    header("location: equipments-list.php");
    exit();
}
...
function execute_equipment_table(){
    ...
    $dataParam     =   [
        ...
        'commissioning_date'    =>  date('Y-m-d h:i:s', strtotime($commissioning_date)),
        'name'          		=>  $name,
        'eel_code'          	=>  $eel_code,
        'origin'          		=>  $origin,
        'capacity'          	=>  $capacity,
...
function execute_equipment_assign_table(){
    global $conn;
    $commissioning_date		= (isset($_POST['commissioning_date']) && !empty($_POST['commissioning_date']) ? trim(mysqli_real_escape_string($conn,$_POST['commissioning_date'])) : date("Y-m-d"));
     ...
     * *****************************rrr_info table operation********************
     */
    $table_sql     =   "equipment_assign";
    $dataParam     =   [
        'eel_code'          	=>  $eel_code,
        'project_id'  			=>  $present_location,
        'assign_date'    =>  date('Y-m-d h:i:s', strtotime($commissioning_date)),
        'remarks'          		=>  $remarks,
...
if (isset($_POST['equipment_update']) && !empty($_POST['equipment_update'])){
    /******************************assets table operation******************** */
    $id		= (isset($_POST['id']) && !empty($_POST['id']) ? trim(mysqli_real_escape_string($conn,$_POST['id'])) : "");
    $rrr_info_response  =   update_equipment_table();
    if(isset($rrr_info_response) && $rrr_info_response['status'] == "success"){
...
if (isset($_POST['equipment_shift'])){
	...
	$present_location_type  = getProjectTypeByID($project_id);
	$assign_date 	= $_POST['assign_date'];
	$remarks 		= $_POST['remarks'];
	$id 			= $_POST['id'];

	$sql	=	"insert into `equipment_assign` values('','$eel_code','$project_id','$assign_date','','$remarks')";
	...
	$sqlUpdate	=	"UPDATE `equipments` SET `present_location`='$project_id' ,`present_location_type`='$present_location_type' WHERE `eel_code`='$eel_code'";
    $responseUpdate	=	mysqli_query($conn, $sqlUpdate);
...
if (isset($_POST['sm_entry']) && !empty($_POST['sm_entry'])){
    ...
    $rrr_info_response  =   execute_maintenance_table();
    if(isset($rrr_info_response) && $rrr_info_response['status'] == "success"){
		$rrr_info_id    =   $rrr_info_response['last_id'];
		$rrr_details_response  =   execute_maintenance_details_table($rrr_info_id);
...
function execute_maintenance_table(){
    global $conn;
...
function execute_maintenance_details_table($rrr_info_id){
    global $conn;
    /*
    ...
    for($count 		= 0; $count<count($_POST['material_name']); $count++){        
        ...
        $dataParam     =   [
            //'id'                	=>  get_table_next_primary_id('maintenance_details'),
            'maintenance_id'       	=>  $rrr_info_id,
        ...
        saveData("maintenance_details", $dataParam);
		...
		/* Update Qty in inv_product_price table*/
				...
				*/
				...
				$material_name	= (isset($_POST['material_name'][$count]) && !empty($_POST['material_name'][$count]) ? trim(mysqli_real_escape_string($conn,$_POST['material_name'][$count])) : '');
				$equipment_id		= (isset($_POST['equipment_id']) && !empty($_POST['equipment_id']) ? trim(mysqli_real_escape_string($conn,$_POST['equipment_id'])) : "");
				$mb_date        = (isset($lastseervice_date) && !empty($lastseervice_date) ? date('Y-m-d h:i:s', strtotime($lastseervice_date)) : date('Y-m-d h:i:s'));
				$mbin_qty       = 0;
				$mbin_val       = 0;
				$mbout_qty      = $quantity;
