...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active"> RLP Reports</li>
    </ol>
    <!-- DataTables Example -->
    <div class="card mb-3">
...
if(isset($_POST['submit'])){
	...
	$resultSet = mysqli_query($conn, "SELECT * FROM `rlp_info` WHERE `is_delete` =  '0'".($project_id!='all'?" AND `request_project` = '$project_id'":'')." AND `request_date` BETWEEN '$from_date' AND '$to_date' ");
...
echo "</pre>"; */
	...
	if($resultSet->num_rows > 0){
		echo "<div id='printableArea'><center><h1 align='center'><img src='images/spl.png' height='50' style='padding-top:10px'></h1><h2>SAIF POWERTEC LIMITED</h2><p>72,Mohakhali C/A, (8th Floor),Rupayan Center,Dhaka-1212,bangladesh</p><h5>RLP List Report</h5>From :";
		...
		while($rows = $resultSet->fetch_assoc()) {
			$i++;
			echo "<tr>
					<td><a href='rlp_view.php?rlp_id=" . $rows['id'] . "' target='blank'>" . $rows['rlp_no'] . "</a></td>
					<td>" . getProjectNameByID($rows['request_project']) . "</td>
					<td>" . date('jS F Y', strtotime($rows['request_date'])) . "</td>
